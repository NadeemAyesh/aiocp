...
@Component({
  ...
  template: `
    ...
    >
      <div 
        class="bg-white rounded-3xl max-w-2xl w-full overflow-hidden shadow-2xl border border-slate-200"
      ...
      >
        ...
        <div class="p-6 sm:p-8 space-y-6">
          ...
          <div class="flex items-center gap-3 pt-2">
            ...
            <button 
              (click)="closeProject()"
              class="px-6 py-3.5 rounded-xl border border-slate-200 text-slate-700 font-bold text-sm hover:bg-slate-50 transition"
            >
              إغلاق
            </button>
          </div>
    ...
    >
      <div 
        class="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-200"
        (click)="$event.stopPropagation()"
      >
        ...
        <div class="p-6 sm:p-8 space-y-6">
          <div class="flex items-center gap-3 text-xs text-slate-400">
            ...
            <span>•</span>
            <span class="text-[#004380] font-bold">{{ selectedArticle.author }}</span>
          </div>

          <h3 class="text-xl sm:text-2xl font-extrabold text-[#00284d] leading-snug">
          ...
          <div class="pt-4 border-t border-slate-100 flex items-center justify-between">
            <button 
              ...
              class="py-3 px-6 rounded-xl bg-[#004380] text-white font-bold text-xs sm:text-sm hover:bg-[#00284d] transition"
            >
              ساهم في دعم مشاريع الإعمار
            </button>
            <button (click)="closeArticle()" class="px-5 py-3 rounded-xl border border-slate-200 text-slate-700 text-xs font-bold hover:bg-slate-50 transition">
              إغلاق
            </button>
          </div>
        </div>
    ...
    >
      <div 
        class="bg-white rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-slate-200"
        (click)="$event.stopPropagation()"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <div class="w-10 h-10 rounded-full bg-orange-100 text-[#f4921e] flex items-center justify-center">
            ...
            <h3 class="text-lg font-bold text-[#00284d]">مشاركة الموقع</h3>
          </div>
          <button (click)="closeShare()" class="text-slate-400 hover:text-slate-700">✕</button>
        </div>
        ...
        <div class="grid grid-cols-4 gap-3 text-center">
          <button (click)="shareTo('whatsapp')" class="p-3 rounded-2xl bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition flex flex-col items-center gap-1.5 font-bold text-xs">
            <span>واتساب</span>
          </button>
          <button (click)="shareTo('facebook')" class="p-3 rounded-2xl bg-blue-50 text-blue-600 hover:bg-blue-100 transition flex flex-col items-center gap-1.5 font-bold text-xs">
            <span>فيسبوك</span>
          </button>
          <button (click)="shareTo('x')" class="p-3 rounded-2xl bg-slate-100 text-slate-800 hover:bg-slate-200 transition flex flex-col items-center gap-1.5 font-bold text-xs">
            <span>منصة X</span>
          </button>
          <button (click)="shareTo('telegram')" class="p-3 rounded-2xl bg-sky-50 text-sky-600 hover:bg-sky-100 transition flex flex-col items-center gap-1.5 font-bold text-xs">
            <span>تيليغرام</span>
          </button>
        </div>
        ...
        <div class="flex items-center gap-2 p-2 bg-slate-50 rounded-xl border border-slate-200">
          <input type="text" [value]="pageUrl" readonly class="flex-1 bg-transparent text-xs text-slate-600 outline-none px-2" dir="ltr" />
    ...
    >
      <div 
        class="bg-white rounded-3xl max-w-xl w-full p-6 space-y-4 shadow-2xl border border-slate-200"
        (click)="$event.stopPropagation()"
      >
        <div class="flex items-center gap-3 border-b border-slate-200 pb-3">
          <svg class="w-5 h-5 text-[#004380]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          <input 
            type="text" 
            [(ngModel)]="searchQuery" 
            placeholder="ابحث عن المشاريع، الأخبار، التقارير الميدانية..." 
            class="flex-1 text-sm font-medium text-slate-800 outline-none"
            autofocus
          />
          <button (click)="closeSearch()" class="text-slate-400 hover:text-slate-700">✕</button>
        </div>

        <div class="space-y-2 max-h-64 overflow-y-auto pt-2">
          <span class="text-[11px] font-bold text-slate-400">روابط مقترحة سريعة:</span>
          <div class="flex flex-wrap gap-2">
            <a href="#activities" (click)="closeSearch()" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition">إزالة الركام</a>
            <a href="#projects" (click)="closeSearch()" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition">مخيمات الإيواء في جباليا</a>
            <a href="#projects" (click)="closeSearch()" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition">تأهيل آبار المياه</a>
            <a href="#media" (click)="closeSearch()" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition">التقرير الهندسي الشامل</a>
            <a href="#about" (click)="closeSearch()" class="px-3 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-xs font-bold text-slate-700 transition">شركاؤنا والأمم المتحدة</a>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      <div 
        class="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-slate-200"
        (click)="$event.stopPropagation()"
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-xl font-extrabold text-[#00284d]">ساهم في إعمار فلسطين</h3>
              <p class="text-xs text-slate-500">مساهمتك تعيد بناء البيوت وترمم الأمل</p>
            </div>
          </div>
          <button (click)="closeDonate()" class="text-slate-400 hover:text-slate-700">✕</button>
        </div>

        <!-- Predefined Amounts -->
        <div class="space-y-2">
          <label class="block text-xs font-bold text-slate-700">اختر قيمة التبرع (USD):</label>
        ...
        <!-- Custom Amount -->
        <div class="space-y-1">
          <label class="block text-xs font-bold text-slate-700">أو أدخل مبلغاً مخصصاً:</label>
          <div class="relative">
            <input 
              type="number" 
              [(ngModel)]="selectedAmount" 
              placeholder="مثال: 300" 
              class="w-full py-3 px-4 rounded-xl border border-slate-200 text-sm font-bold text-[#00284d] outline-none focus:border-[#004380]"
            />
